...
function findFirstSingleChar(str) {
  ...
  const pair = Object.entries(letterCount).find((el) => el[1] === 1);
...
function isCreditCardNumber(ccn) {
  const arr1 = [];
  const arr2 = [];
  const ccnLength = ccn.toString().length;
  const ccnArrR = ccn.toString().split('').reverse();

  for (let i = 0; i < ccnLength; i += 1) {
    if (i % 2 === 0) {
      arr1.push(Number(ccnArrR[i]));
    } else {
      arr2.push(Number(ccnArrR[i]));
    }
  }
  return arr2.map((el) => {
    if (el * 2 > 9) return (el * 2).toString().split('').reduce((acc, num) => Number(acc) + Number(num));
    return el * 2;
  }).concat(arr1).reduce((acc, number) => acc + number) % 10 === 0;
}
...
function getDigitalRoot(num) {
  let sum = num;
  do {
    sum = sum.toString().split('').reduce((acc, el) => +acc + +el);
  } while (sum > 9);
  return sum;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (arr.length === 0
      || BRACKETS_PAIR[str[i]]
      || BRACKETS_PAIR[arr[arr.length - 1]] !== str[i]) {
      arr.push(str[i]);
    } else {
